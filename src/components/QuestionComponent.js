...
export const QuestionComponent = ({question, state, dispatch, product}) => {
    ...
    const handleAnswer = () => {
        dispatch({type: 'ADD_ANSWER', payload: {questionInfo:question, answer, productQInfo:product, productDistri: state.dbMarket.find(user => user.products.some(prod => prod.id === product.id)).userName}});
